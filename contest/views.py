...
class HomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["contests_voting_open"] = Contest.objects.filter(
            submission_close__lte=now, end__gt=now
        )
...
class ContestDetailsView(TemplateView):
    ...
    def get_context_data(self, slug, **kwargs):
        ...
        context["can_submit_photo"] = (
            user_authenticated and contest.submission_close > timezone.now()
        )
        context["can_vote"] = user_authenticated and contest.end > timezone.now()
        context["entries"] = map(
            self.entry_to_dict, Entry.objects.filter(contest=contest).order_by("?")
        )
        return context
...
def contest_upload_photo(request, slug, **kwargs):
    ...
    if timezone.now() >= contest.submission_close:
        return HttpResponseBadRequest("Photo submission period for contest has ended.")
    ...
    Entry.objects.create(contest=contest, photo=photo)
